...
function set_ids(){
    ...
    if [ "${os}" = "Darwin" ]; then
        if [ "${arch}" = "arm64" ]; then
            ...
            neovim_id="https://github.com/neovim/neovim/releases/download/nightly/nvim-macos-arm64.tar.gz"
        else
        ...
        if [ "${arch}" = "x86_64" ]; then
            ...
            neovim_id="https://github.com/neovim/neovim-releases/releases/download/nightly/nvim-linux64.tar.gz"
        else
